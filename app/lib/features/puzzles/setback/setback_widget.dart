...
import 'setback_painter.dart';
import 'setback_provider.dart';

/// Widget that renders and handles interaction for a SETBACK puzzle.
class SetbackPuzzleWidget extends ConsumerStatefulWidget {
  final Map<String, dynamic> gridData;
  final VoidCallback? onSolved;

  const SetbackPuzzleWidget({
    super.key,
  ...
  @override
  ConsumerState<SetbackPuzzleWidget> createState() =>
      _SetbackPuzzleWidgetState();
}

class _SetbackPuzzleWidgetState extends ConsumerState<SetbackPuzzleWidget>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(setbackProvider.notifier).initialize(widget.gridData);
    });
  }
  ...
  Widget build(BuildContext context) {
    final puzzleState = ref.watch(setbackProvider);
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                '${engine.placedCount}/${engine.houseCount} houses',
                style: BauTypography.bodySmall.copyWith(
                  color: engine.placedCount > engine.houseCount
                    ...
                    child: AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Container(
                          ...
                          child: CustomPaint(
                            painter: SetbackPainter(
                              width: engine.width,
                              height: engine.height,
                              cells: engine.cells,
                              conflictCells: puzzleState.conflictCells,
                              isSolved: puzzleState.isSolved,
                              houseCount: engine.houseCount,
                              placedCount: engine.placedCount,
                            ),
                          ),
                        );
                      },
        ...
        Padding(
          ...
          child: TextButton.icon(
            onPressed: () {
              ref.read(setbackProvider.notifier).resetPuzzle();
  ...
  void _handleTap(
    ...
    if (row >= 0 && row < gridHeight && col >= 0 && col < gridWidth) {
      ref.read(setbackProvider.notifier).tapCell(row, col);
    }
  }
}
